...
/* used device driver */
IMPORT	ER	initSIO_scif(SIOCB *, const CFGSIO *, W speed);	// see "driver/sio/sfic.c"
IMPORT	ER	initMemDisk(DISKCB *, const CFGDISK *);


/* memory region definition */
EXPORT	MEMSEG	MemSeg[] = {
	// Flash ROM 2MByte,	CS0, P1
	{0x80000000, 0x80200000, MSA_FROM	/*,	PGA_RO|PGA_C |0x90000000 */},
	// SDRAM, 16Mbyte, 		CS3, P1
	{0x8c000000, 0x8d000000, MSA_RAM	/*,	PGA_RW|PGA_C */},
	// ROM(原則使用しない),	CS0, P2
	{0xa0000000, 0xa4000000, MSA_ROM	/*,	PGA_RO|PGA_NC */},
	// (未使用領域), 		CS1, P2
	{0xa4000000, 0xa8000000, MSA_IO		/*,	PGA_RW|PGA_D |PGA_S|PGA_XN */},
	// WDT(未使用領域含む), CS2, P2
	{0xa8000000, 0xac000000, MSA_IO		/*,	PGA_RW|PGA_D |PGA_S|PGA_XN */},
	// RAM(原則使用しない), CS3, P2
	{0xac000000, 0xb0000000, MSA_RAM	/*,	PGA_RW|PGA_NC */},
	// (未使用領域),		CS4, P1
	{0xb0000000, 0xb4000000, MSA_IO		/*,	PGA_RW|PGA_D |PGA_S|PGA_XN */},
	// OPT(未使用領域),		CS5, P1
	{0xb4000000, 0xb8000000, MSA_IO		/*,	PGA_RW|PGA_D |PGA_S|PGA_XN */},
	// FPGA(未使用領域),	CS6, P1
	{0xb8000000, 0xbc000000, MSA_IO		/*,	PGA_RW|PGA_D |PGA_S|PGA_XN */},
	// 内蔵I/O領域(未使用領域含む), CS7, P4
	{0xe0000000, 0xffffffff, MSA_IO		/*,	PGA_RW|PGA_D |PGA_S|PGA_XN */},

	{0x80000000, 0x80010000, MSA_MON	/*,	0 */},
	{0x80020000, 0x80200000, MSA_RDA	/*, 0 */},
	{0x8c002000, 0x8d000000, MSA_OS		/*,	0 */}
};
...
EXPORT	MEMSEG	NoMemSeg[] = {
	...
	{0x80200000, 0x84000000, 0		/*,		0 */},
	{0x84000000, 0x8c000000, 0		/*,		0 */},
	{0x8d000000, 0x90000000, 0		/*,		0 */},
	{0x90000000, 0xa0000000, 0		/*,		0 */},
	{0xbc000000, 0xc0000000, 0		/*,		0 */},
	{0xc0000000, 0xe0000000, 0		/*,		0 */},
};
...
EXPORT	const	CFGSIO	ConfigSIO[] = {
	{initSIO_scif, 0},			/* "driver/sio/scif.c"を登録 */
};
...
/* boot information */						/* see "cmdsvc/boot.c" */
///EXPORT	const UH	BootSignature = 0xe382;		// signature
EXPORT	const UH	BootSignature = 0x6cc3;		// signature
EXPORT	UB *	const PBootAddr = (UB *)0x8c100000;	// primary boot loader address
...
EXPORT	UW	DipSwStatus(void)
{
	...
	d = PFC.PCDR.BYTE;						// PC#7～PC#0 読み込み

	/* unnecessary bits are masked and then invert logic. */
///	d = (d ^ SW_MON) & SW_MON;
	d = ~d & 0xFF;							// ビット反転＆下位8bitのみ有効

	/* check abort switch */
///	if (in_w(GIO_I(GIO_L)) & 0x00000100) d |= SW_ABT;	/// 未サポート
...
/* LED on/off */
/*
 * Note:下位16bitは点灯指定(0:OFF, 1:ON)、bit1=緑LED, bit0=赤LED
 *      上位16bitのﾏｽｸ値が'1'に対応するLEDの点灯状態は変更されない。
 *
 *      (IOP5:PJ#5=GREEN,PJ#4=RED)
 */
EXPORT	void	cpuLED(UW v)
{
	UB	m, d, r, c;

	m = ~(((v >> 16) << 4) | 0xcf);	// mask (0:unmodified 1:modify)
	d = ((v >>  0) <<4) & 0x30;		// set value (0:off 1:on)

	r = PFC.PJDR.BYTE;				// input PJ port
	c = (r ^ d) & m;				// modify flag (0:unmodified 1:modify)
	PFC.PJDR.BYTE = r ^ c;			// output PJ port
}
