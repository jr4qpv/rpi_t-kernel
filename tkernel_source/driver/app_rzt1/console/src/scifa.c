...
/*
	シリアルコントローラ操作関数群の定義
*/
LOCAL	ER	scifa_in(LINE_INFO *li, UB *buf, W len, W *alen, W tmout);
LOCAL	ER	scifa_out(LINE_INFO *li, UB *buf, W len, W *alen, W tmout);
LOCAL	ER	scifa_ctl(LINE_INFO *li, W kind, UW *arg);
LOCAL	void	scifa_up(LINE_INFO *li);
LOCAL	void	scifa_down(LINE_INFO *li);

EXPORT SC_FUNC	ScFuncSCIFA = {
	scifa_in, scifa_out, scifa_ctl, scifa_up, scifa_down
};

/*
 *	Define the serial controller (RZT1 "SCIFA")
 */
#include "scifa.h"				/* Common definition */
#include "scifasys.h"			/* System-dependent definition */
...
LOCAL	void	rscs_ctrl(LINE_INFO *li)
{
	...
	volatile struct st_scifa *scbase;		// SCIFAベースアドレス

	scbase = (struct st_scifa *)scdefs->c.iobase;

	if (li->flow.csflow) {
		scbase->FCR.WORD = SCFCRx_RTRG_1 | SCFCRx_TTRG_8 | SCFCRx_MCE;
	} else {
		scbase->FCR.WORD = SCFCRx_RTRG_1 | SCFCRx_TTRG_8;
	}
}
...
LOCAL	void	break_ctrl(LINE_INFO *li, W bk)
{
	...
	if (bk) {		// break on
		scbase->SCR.WORD &= ~(SCSCRx_TIE | SCSCRx_TE);	// TE off
		d = scbase->SPTR.WORD;
		d = (d & 0xfffc) | 0x0002;			// SPB2IO=1,SPB2DT=0
		scbase->SPTR.WORD = d;
	} else {		// break off
		scbase->SPTR.BIT.SPB2IO = 0;		// SPB2IO=0
		scbase->SCR.WORD |= (SCSCRx_TIE | SCSCRx_TE);	// TE on
	}
}
...
LOCAL	 ER	init_sio(LINE_INFO *li, RsMode mode)
{
	...
	switch(mode.parity) {
	case 1:	smr |= SCSMRx_PE | SCSMRx_PM_ODD;	break;	// odd
...
LOCAL	void	sio_inthdr_rx(UINT dintno)
{
	...
	for (li = &LineInfo[0], i = nPorts; ; li++) {
		if (--i < 0) return;
		if ( (dintno >= li->scdefs.c.intvec) &&
			 (dintno < (li->scdefs.c.intvec + 4)) ) {	// intvecは代表番号
			if (li->scdefs.fn != &ScFuncSCIFA) return;
			break;
		}
	}
	scdefs = &li->scdefs;
...
///	fctl = scdefs->fctl;		/* FIFO configuration information */
	...
	// ブレーク検出
	if (scbase->FSR.BIT.BRK) {
		// ブレークフラグクリア
		scbase->FSR.BIT.BRK = 0;
		li->lsts     |= SCFSRx_BRK;
		li->lstshist |= SCFSRx_BRK;
	}

	// (エラーの有無に関わらず)受信データ有り
	len = scbase->FDR.WORD & SCFDRx_R_MASK;

	// 受信データ長は長くてもFIFOサイズ
	if (len > SCFDRx_R_SIZE) len = SCFDRx_R_SIZE;
	...
	for (i = 0; i < len; i++) {
		...
		c   = scbase->FRDR;
		ssr = scbase->FSR.WORD;
		scbase->FSR.WORD = ssr & ~(SCFSRx_ER | SCFSRx_RDF | SCFSRx_DR);
	...
	// 受信割込クリア
	scbase->FSR.WORD &= ~(SCFSRx_ER | SCFSRx_RDF | SCFSRx_DR);
	end_inthdr(scdefs);
}
...
///	fctl = scdefs->fctl;		/* FIFO configuration information */
	...
	// 送信完了
	if (scbase->FSR.BIT.TDFE == 0) goto fin;

#ifdef REMOTE_MODE
	// ﾒﾝﾃﾎﾟｰﾄ remote処理
	if ((c = _remote_tx(REMOTE_MODE)) >= 0) {
		scbase->FTDR = c;					// リモート文字を送信
		goto fin;
	}
#endif
	...
	if (li->flowsts.reqchar) {
		scbase->FTDR = li->flowsts.reqchar;
		li->flowsts.reqchar = 0;
		n--;
	}
	if (n > 0) {
		// データ送信
		while(--n >= 0) scbase->FTDR = li->ou_buf[ptr++];
		li->ou_cnt = ptr;
	} else {
...
nosend:
		// 送信割込禁止
		scbase->SCR.BIT.TIE = 0;
	}
fin:
	// 送信割込クリア
	scbase->FSR.BIT.TDFE = 0;
	end_inthdr(scdefs);
}
...
LOCAL	BOOL	send_1st_char(LINE_INFO *li, W c)
{
	...
	// 送信
	scbase->FTDR = c;
	while(--n > 0) scbase->FTDR = li->ou_buf[li->ou_cnt++];

	// 送信割込クリア
	scbase->FSR.BIT.TDFE = 0;

	// 送信割込許可
	scbase->SCR.BIT.TIE = 1;
