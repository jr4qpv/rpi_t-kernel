...
/*
 *	@(#)miniuart.c	Console/Low-level serial I/O driver 2016/09/07
 *
 ...
 */
LOCAL ER	miniuart_in(LINE_INFO *li, UB *buf, W len, W *alen, W tmout);
LOCAL ER	miniuart_out(LINE_INFO *li, UB *buf, W len, W *alen, W tmout);
LOCAL ER	miniuart_ctl(LINE_INFO *li, W kind, UW *arg);
LOCAL void	miniuart_up(LINE_INFO *li);
LOCAL void	miniuart_down(LINE_INFO *li);

EXPORT	SC_FUNC	ScFuncMINIUART = {
	miniuart_in, miniuart_out, miniuart_ctl, miniuart_up, miniuart_down,
};
 ...
 */
#define ON_RTS		set_screg(regMU_MCR, MC_RTS|MC_OUT2)   /* Turn on RTS */
#define OFF_RTS		set_screg2(regMU_MCR, MC_RTS, MC_OUT2) /* Turn off RTS */
#define	ON_BREAK	set_screg(regMU_LCR, LC_SBRK)	/* Break send */
#define	OFF_BREAK	clr_screg(regMU_LCR, LC_SBRK)	/* Break end */

/*
 *	Send/Receive interrupt control
 */

#define	ENB_RXINT(li)	OutW(regMU_IER, (li->enbint == 0) ? \
					0x00 : (IM_LSTS|IM_RCV|IM_MSTS))
#define	ENB_TXRXINT(li)	OutW(regMU_IER, (li->enbint == 0) ? \
					0x00 : (IM_LSTS|IM_SND|IM_RCV|IM_MSTS))
#define	DIS_TXRXINT(li)	OutW(regMU_IER, 0x00)
...
LOCAL	ER	line_ctl( LINE_INFO *li, UW cmd )
{
	...
	switch ( cmd & 0xfffffffc ) {
	  case RSCTL_ON:
		set_screg(regMU_MCR, line | MC_OUT2);
		break;
	  case RSCTL_OFF:
		set_screg2(regMU_MCR, line, MC_OUT2);
		break;
	  case RSCTL_SET:
		set_screg2(regMU_MCR, MC_RTS|MC_DTR, line | MC_OUT2);
		break;
	  default:
		err = E_PAR;
	}
...
LOCAL	ER	init_sio( LINE_INFO *li, RsMode mode )
{
	...
	if ( mode.parity == 3
	  || mode.stopbits == 3
	  || mode.baud < 50 || mode.baud > MU_LINE_SPEED(1) ) return E_PAR;

	/* Communication speed setting value */
	divcnt = MU_LINE_SPEED(mode.baud);

	/* Line mode */
	lctl = 0x00;
	if ( mode.datalen == 0 || mode.datalen == 1) {
		/* 5,6 data bits */
		return E_NOSPT;
	} else {
		/* 7,8 data bits */
		...
		lctl |= mode.datalen & LC_BLEN;
	}
	switch ( mode.parity ) {
	  case 1: return E_NOSPT;						/* Odd parity */
	  case 2: return E_NOSPT;						/* Even parity */
	}
...
//	fctl |= ( mode.baud < 9600 )? FC_TL01: FC_TL08;
	...
	for ( i = 0; i < FIFO_SIZE; ++i ) {
		if ( (InW(regMU_LSR) & LS_TSRE) != 0 ) break;
		tk_dly_tsk(100);
	}
...
//	InB(SC_MSTS);		/* Clear the "IS_MSTS" */
	...
	for ( i = 0; i < FIFO_SIZE; ++i ) {
		InW(regMU_IO);	/* Clear "IS_RCV IS_CTMO" */
	}
...
//	}
	...
	OutW(regMU_BAUD, divcnt);	/* Communication speed */
	OutW(regMU_LCR, lctl);		/* Line mode */
//	OutW(SC_FCTL, (fctl == 0) ? 0 : (fctl|FC_TXCLR|FC_RXCLR));
//					/* Set/Disable the FIFO mode */
	li->msts = InW(regMU_MSR);	/* Initilize the modem status */

	OutW(regMU_MCR, mctl);		/* Modem mode */
...
LOCAL	void	sio_inthdr( UINT dintno )
{
	...
	for (li = &LineInfo[0], i = nPorts; ; li++) {
		...
		if (li->scdefs.c.intvec == dintno) {
			if (li->scdefs.fn != &ScFuncMINIUART) return;
			break;
		}
	}
	scdefs = &li->scdefs;
  ...
  while( CheckInt(scdefs->c.intvec) ) {
   if ( (InW(regAUXIRQ) & AUXIRQ_MUIRQ) == 0 ) continue;
   iir = InW(regMU_IIR) & 0x7;	/* 下位3bit */

   if (iir == IS_RCV) {			/*** 受信関係割り込み? ***/
   ...
   if (iir == IS_SND) {				/*** 送信関係割り込み? ***/
	/* Send processing*/
	li->msts = InW(regMU_MSR);
	lsts = InW(regMU_LSR);
	  ...
	  && (li->flow.csflow == 0 || (li->msts & MS_CS) != 0)	/* CS ON */
		...
		if ( n > 0 ) {			/* There is the send data */
			if ( li->flowsts.reqchar != 0 ) {
				/* Send the "XON / XOFF" */
				OutW(regMU_IO, li->flowsts.reqchar);
...
LOCAL	BOOL	send_1st_char( LINE_INFO *li, W c )
{
	...
	OutW(regMU_IO, c);
	while ( --n > 0 )
		OutW(regMU_IO, li->ou_buf[OU_PTRMASK(li, li->ou_rptr++)]);
	return TRUE;
}
...
LOCAL	ER	miniuart_out( LINE_INFO *li, UB *buf, W len, W *alen, W tmout )
{
	...
	*alen = 0;
	...
	for (wptr = li->ou_wptr; len > 0; ) {
		...
		if ((InW(regMU_IER) & IM_SND) == 0) send_1st_char(li, -1);
...
#|=======================
#|* 2016/08/30	It's made by making reference to "ns16450.c" for [tef_em1d].
#|
*/
