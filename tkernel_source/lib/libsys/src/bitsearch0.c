...
/*
 * Retrieve bit 0 in word-aligned array.
 *	A faster version of BitSearch0 available in case where base is word-aligned.
 *	This implementation uses binary search algorithm for efficiency.
 */
EXPORT W BitSearch0_w( const UW *base, W offset, W width )
{
	UW	*cp, v;
	W	position;
 ...
 */
EXPORT W BitSearch0( const void *base, W offset, W width )
{
#if ALLOW_MISALIGN
	return BitSearch0_w((const UW*)base, offset, width);
#else
	/* Align base to the left word boundary, then use BitSearch0_w */
	UINT	wofs = ((UINT)base) & 3;
	return BitSearch0_w((const UW*)((const UB*)base - wofs), offset + wofs * 8, width);
#endif
}
