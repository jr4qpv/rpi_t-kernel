...
struct GeneralDriverInterface {
	...
	INT		preq[2];/* The number of TDC_READ/WRITE requests during the accept-wait and the processing */

	INT		acpd[2];/* The number of requests of the accept-wait TDC_READ/WRITE (specific data) */
	INT		acpa[2];/* The number of requests of the accept-wait TDC_READ/WRITE (attribute data) */
};

/* Count the number of the accept-wait requests */
#define	cntacpq(req, gdi, ope)					\
	if ( (req)->cmd == TDC_READ ) {				\
		if ( (req)->start < 0 ) (gdi)->acpa[0] ope;	\
		else			(gdi)->acpd[0] ope;	\
	} else {						\
		if ( (req)->start < 0 ) (gdi)->acpa[1] ope;	\
		else			(gdi)->acpd[1] ope;	\
	}
...
/*
 * Lock for DRQ get (gdi_getDRQ)
 */
#define LockDRQ(gdi, rw, tmo)	MLockTmo(&(gdi)->lock, 2 + (rw), (tmo))
#define	UnlockDRQ(gdi, rw)	MUnlock(&(gdi)->lock, 2 + (rw))
...
LOCAL ER gdi_abortreq( ID tskid, T_DEVREQ *req, GDI gdi )
{
	...
	if ( drq->req == NULL ) {
		/* Processing was already completed */
		return E_OK;
	}
 ...
 */
EXPORT ER GDefDevice( const GDefDev *ddev, T_IDEV *idev, GDI *p_gdi )
{
	GDI	gdi;
	DRQ	*drq;
	T_CFLG	cflg;
	...
	if ( GDI_TEV == 0 ) {
		...
		n = tk_get_cfn((UB*)"TEV_GDI", &GDI_TEV, 1);
	...
	gdi->preq[0] = gdi->preq[1] = 0;
	gdi->acpd[0] = gdi->acpd[1] = 0;
	gdi->acpa[0] = gdi->acpa[1] = 0;
	...
	STRNCPY((B*)&cflg.exinf, (B*)gdi->def.devnm, sizeof(void*));
 ...
 */
EXPORT ER GRedefDevice( const GDefDev *ddev, GDI gdi )
{
	DRQ	*drq;
	ER	err;
