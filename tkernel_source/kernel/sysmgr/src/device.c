...
EXPORT	DevCB	*DevCBtbl;	/* Device registration information table */
EXPORT	QUEUE	UsedDevCB;	/* In-use queue */
LOCAL	QUEUE	FreeDevCB;	/* Unused queue */
LOCAL	INT	MaxRegDev;	/* Maximum number of device registrations */
...
EXPORT DevCB* searchDevCB( CONST UB *devnm )
{
	...
	for ( q = UsedDevCB.next; q != &UsedDevCB; q = q->next ) {
		devcb = (DevCB*)q;
		if ( devcb->devnm[0] == devnm[0] &&
			STRCMP((char*)devcb->devnm, (char*)devnm) == 0 ) {
			return devcb; /* Found */
		}
	}
 ...
 * Device registration
 */
LOCAL ID _tk_def_dev( CONST UB *devnm, CONST T_DDEV *ddev,
					T_IDEV *idev, void *caller_gp )
{
	DevCB	*devcb;
	INT	len, evttyp;
	ER	ercd;
...
LOCAL INT _tk_lst_dev( T_LDEV *ldev, INT start, INT ndev )
{
	...
	for ( q = UsedDevCB.next; q != &UsedDevCB; q = q->next ) {
		if ( n >= start && n < end ) {
			...
			STRNCPY((char*)ldev->devnm,
					(char*)devcb->devnm, L_DEVNM);
			ldev++;
		}
		n++;
	}
...
LOCAL INT _tk_evt_dev( ID devid, INT evttyp, void *evtinf )
{
	...
	/* Device driver call */
	ercd = call_eventfn(devcb, evttyp, evtinf);
...
EXPORT ER initialize_devmgr( void )
{
	...
	ercd = CreateMLock(&DevMgrLock, (UB*)OBJNAME_DMLOCK);
	if ( ercd < E_OK ) {
		goto err_ret;
	}
...
EXPORT ER finish_devmgr( void )
{
	...
	ERCD_PRINT(("1. finish_devmgr -> tk_def_ssy ercd = %d\n", ercd));

	/* Unregister device initial setting information */
	ercd = delIDev();
	ERCD_PRINT(("2. finish_devmgr -> delIDev ercd = %d\n", ercd));

	/* Finalization sequence of device input/output-related */
	ercd = finishDevIO();
	ERCD_PRINT(("3. finish_devmgr -> finishDevIO ercd = %d\n", ercd));
