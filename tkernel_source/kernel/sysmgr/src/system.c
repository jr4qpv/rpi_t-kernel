 ...
 *	Called directly from sysinit.
 */
EXPORT INT getcfn( CONST UB *conf, CONST UB *name, INT *val, INT max )
{
	CONST UB	*p;
	INT		v, n;
 ...
 *	Called directly from sysinit.
 */
EXPORT INT getcfs( CONST UB *conf, CONST UB *name, UB *bp, INT max )
{
	CONST UB	*p, *sp;
	UB		c;
...
err_ret:
	TM_DEBUG_PRINT(("_tk_get_cfn ercd = %d\n", n));
	return n;
}
...
err_ret:
	TM_DEBUG_PRINT(("_tk_get_cfs ercd = %d\n", n));
	return n;
}
