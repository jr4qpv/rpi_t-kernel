...
//	tm_monitor();
	while (1) {
		int i;
		for (i=500; i>0; i--) WaitUsec(1000);	// 0.5秒待ち
		tm_putchar('a');
		if ((n++) & 1)
			tm_extsvc(TMEF_LED, 0x0001, 0, 0);	// LED on
		else
			tm_extsvc(TMEF_LED, 0x0000, 0, 0);	// LED off
	}
#endif
...
/* ------------------------------------------------------------------------
#|History of "devinit.c"
#|=====================
#|* 2016/06/05	It's copied from "../tef_em1d/" and it's modified.
#|
*/
