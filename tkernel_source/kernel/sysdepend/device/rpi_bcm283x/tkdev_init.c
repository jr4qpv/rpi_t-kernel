...
/*
 *	@(#)tkdev_init.c (BCM283x) 2016/07/21
...
{
	/* すべての割り込み禁止 */
	out_w(ARM_IRQ_DIBL1, 0xffffffff);
	out_w(ARM_IRQ_DIBL2, 0xffffffff);
	out_w(ARM_IRQ_DIBL3, 0xffffffff);
	out_w(ARM_IRQ_FAST, 0);
...
/*----------------------------------------------------------------------
#|History of "tkdev_init.c"
#|=========================
#|* 2016/07/21	[rpi_bcm283x]用に、[tef_em1d]用の"tkdev_init.h"を参考に作成。
#|
*/
